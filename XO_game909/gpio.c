...
void GPIO_DigitalEnable(GPIO_PORT portNum, GPIO_PIN pinNum, Digital_Enable DigitalEnable ) {
	switch (portNum) {
		...
		case GPIOB:
			if (DigitalEnable == ENABLE) {
				SET_BIT(GPIO_PORTB_DEN_R,pinNum);	
			}
			else {
				CLEAR_BIT(GPIO_PORTB_DEN_R,pinNum);
			}
				break;
			
		case GPIOC:
			if (DigitalEnable == ENABLE) {
				SET_BIT(GPIO_PORTC_DEN_R,pinNum);	
			}
			else {
				CLEAR_BIT(GPIO_PORTC_DEN_R,pinNum);
			}
				break;
			
		case GPIOD:
			if (DigitalEnable == ENABLE) {
				SET_BIT(GPIO_PORTD_DEN_R,pinNum);	
			}
			else {
				CLEAR_BIT(GPIO_PORTD_DEN_R,pinNum);
			}
		case GPIOE:
			if (DigitalEnable == ENABLE) {
				SET_BIT(GPIO_PORTE_DEN_R,pinNum);	
			}
			else {
				CLEAR_BIT(GPIO_PORTE_DEN_R,pinNum);
	          }
				break;
						
	    case GPIOF:
			if (DigitalEnable == ENABLE) {
				SET_BIT(GPIO_PORTF_DEN_R,pinNum);	
			}
			else {
				CLEAR_BIT(GPIO_PORTF_DEN_R,pinNum);
	          }
			break;
			}
}
...
void GPIO_SelectInterruptEvent(GPIO_PORT portNum, GPIO_PIN pinNum, uint8 Event)
{
	switch(portNum)
	{
		...
		case GPIOB:
			if (Event == RISING_EDGE)
			{
				SET_BIT(GPIO_PORTB_IEV_R,pinNum);
			}
			else
			{
				CLEAR_BIT(GPIO_PORTB_IEV_R, pinNum);
			}
			break;
		case GPIOC:
			if (Event == RISING_EDGE)
			{
				SET_BIT(GPIO_PORTC_IEV_R,pinNum);
			}
			else
			{
				CLEAR_BIT(GPIO_PORTC_IEV_R, pinNum);
			}
			break;
		case GPIOD:
			if (Event == RISING_EDGE)
			{
				SET_BIT(GPIO_PORTD_IEV_R,pinNum);
			}
			else
			{
				CLEAR_BIT(GPIO_PORTD_IEV_R, pinNum);
			}
			break;
		case GPIOE:
			if (Event == RISING_EDGE)
			{
				SET_BIT(GPIO_PORTE_IEV_R,pinNum);
			}
			else
			{
				CLEAR_BIT(GPIO_PORTE_IEV_R, pinNum);
			}
			break;
		case GPIOF:
			if (Event == RISING_EDGE)
			{
				SET_BIT(GPIO_PORTF_IEV_R,pinNum);
			}
			else
			{
				CLEAR_BIT(GPIO_PORTF_IEV_R, pinNum);
			}
			break;
	}
}
...
void GPIO_InterruptBothEdges(GPIO_PORT portNum, GPIO_PIN pinNum, uint8 BothEdges)
{
	switch(portNum)
	{
		...
		case GPIOB:
			if (BothEdges == BOTH_EDGE)
			{
				SET_BIT(GPIO_PORTB_IBE_R,pinNum);
			}
			else
			{
				CLEAR_BIT(GPIO_PORTB_IBE_R, pinNum);
			}
			break;
		case GPIOC:
			if (BothEdges == BOTH_EDGE)
			{
				SET_BIT(GPIO_PORTC_IBE_R,pinNum);
			}
			else
			{
				CLEAR_BIT(GPIO_PORTC_IBE_R, pinNum);
			}
			break;
		case GPIOD:
			if (BothEdges == BOTH_EDGE)
			{
				SET_BIT(GPIO_PORTD_IBE_R,pinNum);
			}
			else
			{
				CLEAR_BIT(GPIO_PORTD_IBE_R, pinNum);
			}
			break;
		case GPIOE:
			if (BothEdges == BOTH_EDGE)
			{
				SET_BIT(GPIO_PORTE_IBE_R,pinNum);
			}
			else
			{
				CLEAR_BIT(GPIO_PORTE_IBE_R, pinNum);
			}
			break;
		case GPIOF:
			if (BothEdges == BOTH_EDGE)
			{
				SET_BIT(GPIO_PORTF_IBE_R,pinNum);
			}
			else
			{
				CLEAR_BIT(GPIO_PORTF_IBE_R, pinNum);
			}
			break;
	}
}
...
{ 
  ...
  delay = SYSCTL_RCGC2_R;         // delay
	GPIO_PORTF_LOCK_R= 0x4C4F434B;  // 2)unlock GPIO of PORTF
  GPIO_PORTF_CR_R = 0x1F;         // allow changes to PF4-0
	
  GPIO_PORTF_AMSEL_R = 0x00;      // 3) disable analog function
  GPIO_PORTF_PCTL_R = 0x00000000; // 4) GPIO clear bit PCTL
	//GPIO_PORTF_DIR_R = 0x00 ;       // 5) PORT direction
  GPIO_PORTF_AFSEL_R = 0x00;      // 6) no alternate function
	//GPIO_PORTF_DEN_R = 0x1F;      //     enable digital I/O on PF4
	
  GPIO_PORTF_PUR_R |= 0x1F;     //     enable weak pull-up on PF4
  GPIO_PORTF_IS_R |=0x00;     // (d) PF4 & PF0 is edge-sensitive
  GPIO_PORTF_IBE_R |=0x00;    //     PF4 & PF0 is not both edges
  GPIO_PORTF_IEV_R |=0x00;    //     PF4 & PF0 falling edge event
	
	GPIO_PORTF_ICR_R |= 0x1F;      // (e) clear PortF flags
  GPIO_PORTF_IM_R   |= 0x1F;      // (f) arm interrupt on PortF
...
void PortB_Init(void)
{
  ...
  GPIO_PORTB_CR_R = 0x1E;         //allow changes to PB4-1
  GPIO_PORTB_AMSEL_R = 0x00;      // 3) disable analog function
  GPIO_PORTB_PCTL_R = 0x00000000; // 4) GPIO clear bit PCTL
  GPIO_PORTB_DIR_R = 0x00 ;       // 5) PORT direction
  GPIO_PORTB_AFSEL_R = 0x00;      // 6) no alternate function
  GPIO_PORTB_DEN_R = 0x00;        // 7) enable digital 
}
